...
2. **Built-in packs** (opt-in): JSON files in `builtin-packs/`, loaded when their name appears in `llmSlopDetector.enabledPacks`. Current packs: `academic`, `cliches`, `puffery`, `fiction`, `claudeisms`, `structural`, `security`, `openai`, `gemini`, `deepseek`, `llama`, `qwen`, `grok`, `perplexity`. Whitelist is in `BUILTIN_PACKS` in `src/core/rules.ts` -- unknown names are ignored. Attribution for each pack (where required) is in `THIRD_PARTY_NOTICES.md`.
...
5. Same workflow then builds the vsix, uploads it to the GitHub Release, publishes to the VS Code Marketplace under publisher `thias-se`, and publishes to the Open VSX Registry (for VSCodium and other non-Microsoft forks) under the same namespace, and publishes the CLI + MCP server package to npm.
...
**Browser-extension publishing is automated too.** The same workflow has a `publish-browser` job that runs `npm run package:browser`, attaches the zip to the GitHub Release, and submits it to the Chrome Web Store (`chrome-webstore-upload-cli`, auto-publish), Edge Add-ons (`wdzeng/edge-addon`), and Firefox AMO (`web-ext sign` on the listed channel, with a `git archive` source zip because the bundle is minified). Each store still reviews before the update goes live. Every publish step is `continue-on-error`, so check the job's step results after a release rather than only its overall status. Store credentials live in the `CHROME_*`, `EDGE_*`, and `AMO_*` repo secrets. `docs/STORE_SUBMISSION.md` holds the listing metadata, screenshot list, and per-store review timelines for the rare manual resubmission. The web playground deploys automatically to GitHub Pages via `.github/workflows/pages.yml`.
